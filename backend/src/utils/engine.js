...
const ENGINE_STATUS_ALIASES = new Map([
...
const parseEngineStatus = (status) => ENGINE_STATUS_ALIASES.get(normalizeText(status)) || null;

const normalizeEngineStatus = (status, fallback = ENGINE_STATUS.OPERATIONAL) => (
  parseEngineStatus(status) || fallback
);

const isOperationalEngineStatus = (status) => (
  parseEngineStatus(status) === ENGINE_STATUS.OPERATIONAL
);
...
const normalizeEngineName = (name) => String(name || '')
...
const normalizeEngineParameterName = (name) => ENGINE_PARAMETER_NAMES.get(name) || name;

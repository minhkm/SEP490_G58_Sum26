...
async function seed() {
  ...
  try {
    ...
    // VQS 01/2026 - Manila
    const cVQS01 = await Cargo.create({ voyageId: vVQS01.id, cargoName: 'Vietnam White Rice 5% Broken', cargoType: 'Rice', totalWeight: 2750.0, status: 'Discharged' }, { transaction: t });
    await CargoItem.create({ cargoId: cVQS01.id, itemName: 'White Rice 25KG Bags', quantity: 110000, weight: 2750.0 }, { transaction: t });
    ...
    const cVQS02 = await Cargo.create({ voyageId: vVQS02.id, cargoName: 'Ship Stores & Provisions', cargoType: 'Stores', totalWeight: 15.5, status: 'Discharged' }, { transaction: t });
    await CargoItem.create({ cargoId: cVQS02.id, itemName: 'Food & Provisions', quantity: 1, weight: 15.5 }, { transaction: t });
    ...
    await CargoItem.bulkCreate([
      { cargoId: cVQS03.id, itemName: 'Sweet Hasmin Rice 25KG Bags (Good Order)', quantity: 111806, weight: 2795.15 },
      { cargoId: cVQS03.id, itemName: 'Busted/Broken Bags',                        quantity: 108,    weight: 2.70   },
      { cargoId: cVQS03.id, itemName: 'Wet Bags',                                  quantity: 36,     weight: 0.90   },
      { cargoId: cVQS03.id, itemName: 'Empty Broken Bags',                          quantity: 50,     weight: 0      },
    ], { transaction: t });
    ...
    await CargoItem.bulkCreate([
      { cargoId: cS6601.id, itemName: 'Long Grain Rice 50KG New PP Bags (Good Order)', quantity: 55935, weight: 2796.75 },
      { cargoId: cS6601.id, itemName: '0.5% Empty Bags Allowance',                      quantity: 280,   weight: 0      },
    ], { transaction: t });
